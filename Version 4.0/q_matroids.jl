...
struct Q_Matroid
    groundspace::Any           # groudspace as id-matrix over the corresponding field.
    bases::AbstractVector{Any} # bases of the q-matroid.
end
...
@doc raw"""
    q_matroid_from_independentspaces(Indeps::Any, show_bases=false::Bool)
...
function q_matroid_from_independentspaces(Indeps::Any, show_bases=false::Bool)   
    ...
    bases = AbstractVector{Any}([x for x in Sorted_indeps])
...
@doc raw"""
    q_matroid_from_dependentspaces(Deps::AbstractVector{Any}, show_bases=false::Bool)
...
function q_matroid_from_dependentspaces(Deps::Any, show_bases=false::Bool)
    field = base_ring(Deps[1])
    dim = ncols(Deps[1])
    all_sp = all_subspaces(field,dim)

    Indeps = AbstractVector{Any}([x for x in all_sp if !(x in Deps)])
...
@doc raw"""
    q_matroid_from_circuits(Circs::Any, show_bases=false::Bool)
...
function q_matroid_from_circuits(Circs::Any, show_bases=false::Bool)
    Deps = AbstractVector{Any}([])
...
@doc raw"""
    q_matroid_from_hyperplanes(Hyperps::Any, show_bases=false::Bool)
...
function q_matroid_from_hyperplanes(Hyperps::Any, show_bases=false::Bool)
    # Create the dual circs and the dual q-matroids
    Dual_Circs = AbstractVector{Any}([orthogonal_complementV2(x)[1] for x in Hyperps])
...
@doc raw"""
    q_matroid_from_matrix(Mat::Any)

    Construct a `q-matroid` from a Matrix that represents it.
""" 

function q_matroid_from_matrix(Mat::Any)
    field = base_ring(Mat) 
    n = ncols(Mat)
    k = rank(Mat) 
...
function Rank_polytope(q::Oscar.IntegerUnion, n::Oscar.IntegerUnion)
    ...
    for combi in Oscar.combinations(labeled_all_sp,2)
    ...
    return Polyhedron((A,b)), labeled_all_sp,A,b
...
function Uniform_q_matroid(field::Any, k::Oscar.IntegerUnion, n::Oscar.IntegerUnion)
...
@doc raw"""
    Are_q_matroid_dependentspaces(Deps::Any, choice=nothing::Union{Nothing,String})
...
function Are_q_matroid_dependentspaces(Deps::Any, choice=nothing::Union{Nothing,String})
    field = base_ring(Deps[1])
    are_deps = true
    loop_breaker = true
    fail = "Non"
    ...
    if Deps == []
        ...
        # Check axiom (D3)
        if are_deps
            for pair in Oscar.combinations(collect(values(deps_dict)),2)
                if loop_breaker
...
@doc raw"""
    Are_q_matroid_hyperplanes(Hyperp::Any, choice=nothing::Union{Nothing,String})
...
function Are_q_matroid_hyperplanes(Hyperps::Any, choice=nothing::Union{Nothing,String})
    ...
    if Hyperps == []
        ...
        if are_hyperps
            for combi in Oscar.combinations(Hyperps_sets,2)
        ...
        if are_hyperps
            ...
            loop_breaker = false
            for combi in Oscar.combinations(Hyperps,2)
                if !(loop_breaker)
...
function  Are_q_matroid_bases(Bases::Any)
    ...
    if Set(Bases) == Set(Uniform_q_matroid(field,q_rank,dim).bases)
        ...
        # Check (nB3) for distinct B_1, B_2
            loop_breaker = true
            for pair in Oscar.combinations(collect(values(Info_dict_bases)),2)
                if loop_breaker
                    for elm in pair[1][3]
                        count = 0
                        for tuple in Dict_codim_one_E
...
@doc raw"""
    Restriction_Q_Matroid(QM::Q_Matroid, restriction_space::Any)
...
function Restriction_Q_Matroid(QM::Q_Matroid, restriction_space::Any)
    ...
    Indeps_restriction_space = AbstractVector{Any}([])
    ...
    Bases_restriction_space = AbstractVector{Any}([x for x in Sorted_indeps])
